...
import ACTION_ADMIN_LOGIN_PASSWORD_INVALID from "../../../actions/Admin/AdminLogin/AdminPassword/Invalid/ACTION_ADMIN_LOGIN_PASSWORD_INVALID";
...
const AdminLoginPage = (props) => {
  ...
  useMemo(() => {
    if (getEmployeesData) {
      for (let i = 0; i < getEmployeesData.employees.length; i++) {
        if (getEmployeesData.employees[i].email === adminLoginEmail) {
          if (getEmployeesData.employees[i].password !== null) {
            changeRegisteredEmployeeFound(true);
          } else {
            if (registeredEmployeeFound) {
              changeRegisteredEmployeeFound(false);
            }
          }
        }
      }
    }
  }, [getEmployeesData, adminLoginEmail, registeredEmployeeFound]);
  ...
  return (
    <div className="admin_login_page_background">
      <Modal
